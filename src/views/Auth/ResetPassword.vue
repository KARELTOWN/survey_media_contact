...
import { fetchPatch } from '@/composables/request'
...
const { validatePassword } = authValidator()
const router = useRouter()
const schemaRegister = validatePassword()
...
const handleSubmit = async () => {
  try {
    ...
    const result = await fetchPatch(`auth/reset-password`, data)
    ...
    if (response.status === true) {
      ...
      if (response?.data) {
        successNotify('Mot de passe réinitialisé avec succès.')
        setTimeout(() => {
