            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th class="px-5 py-3 text-left w-2/11 sm:px-6">
                            <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Direction</p>
                        </th>
                        <th class="px-5 py-3 text-left w-2/11 sm:px-6">
                            <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Fonction</p>
                        </th>
                        <th class="px-5 py-3 text-left w-2/11 sm:px-6">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="(user, index) in users" :key="index"
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ user.firstname + ' ' +
                                user.lastname }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ user.direction_id ?
                                user.direction_id.libelle : '---' }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ user.fonction_id ?
                                user.fonction_id.libelle : '---' }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ user.role_id ?
                                user.role_id.libelle : '---' }}</p>
                        </td>
                        <td>
                            <div class="flex flex justify-center space-x-2">
                                <Button @click="changeAccountStatus(user, index)" size="sm" variant="outline"
...
const changeAccountStatus = async (user, index) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            await changeStatus({ user_id: user._id })
